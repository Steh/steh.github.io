...
-[search command: walklex](https://help.splunk.com/en/splunk-enterprise/search/spl-search-reference/10.0/search-commands/walklex) 

### return only indexed field names

When you need to know which fields you can use with tstats.

```bash
| walklex index=* type=field
| stats values(field) AS indexed_fields by index
```

### return the set of terms that are indexed values

```bash
| walklex index=* type=fieldvalue
...
```bash
| walklex index=* type=term
| stats sum(count) by term index
```

### return all

```bash
| walklex index=* type=all
| stats sum(count) by term index
```

## create new fieldnames with variables

```bash
index=main sourcetype=access_combined
| eval Total_Count_of_{host} = host
| stats count(Total_Count_of_*) as Total_Count_of_*
```
* i don't know if this search would work
